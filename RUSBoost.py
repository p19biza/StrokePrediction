...
rusb = RUSBoostClassifier(n_estimators=200, random_state=42, algorithm='SAMME.R')

# Train the classifier on the training data
rusb.fit(X_train, y_train)

# Perform a 10-fold cross-validation on the training data
scores = cross_val_score(rusb, X_train, y_train, cv=10)

# Make predictions on the test data
y_pred = rusb.predict(X_test)
...
plt.title('Feature Importance - RUSBoost')
